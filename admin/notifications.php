...
$query = "
    SELECT 
        nl.*,
        -- Get sender/admin info (user who triggered the notification)
        sender.username as sender_username,
        sender.full_name as sender_full_name,
        sender.role as sender_role,
        -- Get recipient user info (if recipient_user_id points to a user who received it)
        recipient.username as recipient_username,
        recipient.full_name as recipient_full_name,
        recipient.role as recipient_role
    FROM notifications_log nl
    -- Left join for sender (the user who triggered/created the notification)
    LEFT JOIN users sender ON nl.recipient_user_id = sender.id
    -- Left join for recipient using recipient_user_id (if it's a user ID of the recipient)
    LEFT JOIN users recipient ON nl.recipient_user_id = recipient.id
    WHERE 1=1
";
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-change">
                        ...
                        <?= $stats['total'] > 0 ? round(($stats['sent'] / $stats['total']) * 100, 1) : 0; ?>% success
                        rate
                    </div>
                </div>
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-change">
                        ...
                        <?= $stats['total'] > 0 ? round(($stats['failed'] / $stats['total']) * 100, 1) : 0; ?>% failure
                        rate
                    </div>
                </div>
...
<td>
    <div class="recipient-info">
        <!-- Show phone/email from notifications_log -->
        <?php if ($row['recipient_phone'] || $row['recipient_email']): ?>
            <div class="recipient-name">
                <?php if ($row['recipient_phone']): ?>
                    <i class="fas fa-phone"></i> <?= htmlspecialchars($row['recipient_phone']); ?>
                <?php else: ?>
                    <i class="fas fa-envelope"></i> <?= htmlspecialchars($row['recipient_email']); ?>
                <?php endif; ?>
            </div>
        <?php endif; ?>
        
        <!-- Show user info if available -->
        <?php if (isset($row['full_name']) && !empty($row['full_name']) || isset($row['username']) && !empty($row['username'])): ?>
            <div class="recipient-details">
                ...
                ?>
                <?php if (isset($row['role']) && !empty($row['role'])): ?>
                    · <?= ucfirst($row['role']); ?>
                <?php endif; ?>
            </div>
        <?php elseif (isset($row['recipient_user_id']) && !empty($row['recipient_user_id'])): ?>
            <div class="recipient-details">
                User ID: <?= htmlspecialchars($row['recipient_user_id']); ?>
            </div>
        <?php endif; ?>
    </div>
</td>
